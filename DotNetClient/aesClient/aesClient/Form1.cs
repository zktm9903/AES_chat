...
namespace aesClient
{
    public partial class Form1 : Form
    {
        ...
        public string crypthion(char type, string txt)
        {
            ...
            //CMD에 보낼 명령어를 입력
            pro.StandardInput.Write(@"cd C:\\Users\\Owner\\source\\repos\\openssl_aes\\x64\\Debug" + Environment.NewLine); //경로 설정

            if(type == 'E')
                pro.StandardInput.Write(@"openssl_aes.exe E " + "\"" + txt + "\"" + Environment.NewLine);
            else if (type == 'D')
                pro.StandardInput.Write(@"openssl_aes.exe D " + "\"" + txt + "\"" + Environment.NewLine);
